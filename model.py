...
def get_G(input_shape):
    ...
    nin = Input(input_shape)     # --------------------------------> Input layer
    n = Conv2d(64, (3, 3), (1, 1), act=tf.nn.relu, padding='SAME', W_init=w_init)(nin) # -----------> Gen 1: add pre-residual block
    temp = n

    # B residual blocks          ------------------------> Add 16 residual block
    for i in range(16):
    ...
    n = Conv2d(64, (3, 3), (1, 1), padding='SAME', W_init=w_init, b_init=None)(n) # ------------> Add the post-residual block
    n = BatchNorm2d(gamma_init=g_init)(n)
    n = Elementwise(tf.add)([n, temp])
    
    # B residual blocks end --------------------------------------------> Add an upsampling block
